...
size_t gzip_wrap(const unsigned char* deflate_data, size_t deflate_len,
    ...
    size_t pos=0;
    out[pos++]=0x1f; out[pos++]=0x8b; out[pos++]=8; out[pos++]=0;
    out[pos++]=0; out[pos++]=0; out[pos++]=0; out[pos++]=0;
    out[pos++]=0; out[pos++]=255;
    memcpy(out+pos, deflate_data, deflate_len); pos += deflate_len;
...
size_t gzip_compress_fixed(const unsigned char* in, size_t n, unsigned char* out, size_t outcap){
    lz77_item_t* items = (lz77_item_t*)malloc(sizeof(lz77_item_t)*n); if(!items) return 0;
    size_t nitems = lz77_greedy(in,n,items);
    unsigned char* tmp = (unsigned char*)malloc(n*2+1024); if(!tmp){ free(items); return 0; }
    size_t deflen = deflate_fixed_block(items, nitems, tmp, n*2+1024);
    if(!deflen){ free(items); free(tmp); return 0; }
    size_t wrote = gzip_wrap(tmp, deflen, in, n, out, outcap);
    free(items); free(tmp); return wrote;
}
